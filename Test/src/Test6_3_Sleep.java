...
public class Test6_3_Sleep {
	   ...
	   public static void main(String[] args) throws Exception {
	    	...
	    	//System.setProperty(org.slf4j.impl.SimpleLogger.DEFAULT_LOG_LEVEL_KEY, "DEBUG");
	  		final Logger log = LoggerFactory.getLogger(Test6_3_Sleep.class);
		    ...
		    */
	    	...
	    	DMeta meta = new DMeta("localhost", "5000");
	     	...
	     	// 60s, 3 splits, 1 CS = 121 s
	     	// 3*60s, 3 splits, 1 CS = 361 s
	     	// 5*60s, 3 splits, 1 CS = 601 s
	     	// 10*60s, 3 splits, 1 CS =  1202 s
	     	
	     	// 60s, 3 splits, 2 CS =  s
	     	// 3*60s, 3 splits, 2 CS =  s
	     	// 5*60s, 3 splits, 2 CS =  s
	     	// 10*60s, 3 splits, 2 CS = 602 s
	     	
	     	// Observation : It seems each CS handles 2 CU concurrently for CS of 5-thread configuration
	     	
	     	DComputeUnit cu = new DComputeUnit() {
	     		public String compute(DComputeContext jsonInput) {
	     			System.out.println("Hello from this Compute Unit CU #" + jsonInput.getCU());
	     			System.out.println("Going to sleep for unit : " + jsonInput.getCU());
	     			try {
	     				Thread.sleep(10*60*1000);
	     			} catch (Exception e) { 
	     				return "Error for unit : " + jsonInput.getCU();
	     			}
	     			System.out.println("Waking from sleep for unit : " + jsonInput.getCU());
	     	...
	     	c.executeObject(cu).input(jsonInput).split(3).maxRetry(3).output(out1);
	     	c.perform();
	     	c.finish();
