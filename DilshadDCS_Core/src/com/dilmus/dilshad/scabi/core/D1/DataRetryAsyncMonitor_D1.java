 ...
 * All Rights Reserved.
 * Created 18-Jul-2016
 * File Name : DataRetryAsyncMonitor_D1.java
 */
...
public class DataRetryAsyncMonitor_D1 implements Runnable {
	private final Logger log = LoggerFactory.getLogger(DataRetryAsyncMonitor_D1.class);
	...
	private LinkedList<DataAsyncRun_D1> m_retryCRunList = null;
	private LinkedList<DataAsyncRun_D1> m_allowedCRunList = null;
	
	private long m_cnbListSize = 0; // TODO change to long
	
	public DataRetryAsyncMonitor_D1(Data compute, DMeta meta) {
		m_compute = compute;
		m_meta = meta;
		...
		m_retryCRunList = new LinkedList<DataAsyncRun_D1>();
		m_allowedCRunList = new LinkedList<DataAsyncRun_D1>();
		
		m_cnbListSize = compute.getCNBListSize();
	}
	
	List<DataNoBlock> getCNBList() {
		return m_cnbList;
		// Previous works return m_localCNBList;
	}
	
	private int retryShuffle(int commandId, DataAsyncRun_D1 crun) {
	...
	private int retryShuffle(int commandId, DataAsyncRun_D1 errorReporterCrun, DataAsyncRun_D1 errorCrun) {
	...
	public int executeRetry() {
		...
		while (check) {
			...
			for (DataAsyncRun_D1 crun : m_retryCRunList) {
			...
			*/
			...
			for (DataAsyncRun_D1 crun : m_allowedCRunList) {
	...
	public int doRetry() throws ParseException, IOException, DScabiClientException {
		...
		*/
		...
		for (DataAsyncRun_D1 crun : m_crunList /* works m_localCRunList*/) {
		...
		*/
		...
		for (DataAsyncRun_D1 crun : m_crunList /* Previous works m_localCRunList*/) {
