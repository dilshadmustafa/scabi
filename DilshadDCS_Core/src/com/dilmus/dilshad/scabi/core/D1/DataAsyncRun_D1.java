 ...
 * All Rights Reserved.
 * Created 18-Jul-2016
 * File Name : DataAsyncRun_D1.java
 */
...
import com.dilmus.dilshad.scabi.core.data.IConfig;
...
public class DataAsyncRun_D1 implements Runnable {

	private final Logger log = LoggerFactory.getLogger(DataAsyncRun_D1.class);
		...
		*/
		...
		try {
			...
			if (m_config instanceof DataUnitConfig) {
				/* remove
				if (DataUnitConfig.OBJECT == m_config.getConfigType()) {
					log.debug("doRun() Executing for Object");
					m_futureHttpResponse = m_computeNB.executeObject(((DataUnitConfig)m_config).getDataUnitObject());
					
				} else if (DataUnitConfig.CLASS == m_config.getConfigType()){
					log.debug("doRun() Executing for Class");
					m_futureHttpResponse = m_computeNB.executeClass(((DataUnitConfig)m_config).getDataUnitClass());
				} 
				*/
				...
				if (DPartitionerConfig.OBJECT == m_config.getConfigType()) {
					log.debug("doRun() Executing for Object");
					// m_futureHttpResponse = m_computeNB.executeObject(((DPartitionerConfig)m_config).getPartitionerObject());
				} else if (DPartitionerConfig.CLASS == m_config.getConfigType()){
					log.debug("doRun() Executing for Class");
					m_futureHttpResponse = m_computeNB.executeClass(((DPartitionerConfig)m_config).getPartitionerClass());
				} 
				*/
				log.debug("doRun() Executing for Partitioner");
				//m_futureHttpResponse = m_computeNB.executeForPartitioner(m_commandMap);
			}
			
			/* remove
			else if (DataAsyncConfig.CODE == m_config.getConfigType()){
				log.debug("doRun() Executing for Code");
				m_futureHttpResponse = m_computeNB.executeCode(m_config.getComputeCode());
				
			} else if (DataAsyncConfig.CLASSNAMEINJAR == m_config.getConfigType()){
	...
	public void run() {
		...
		if (m_startCommandId <= 0) {
			log.debug("run() m_startCommandId is not set. m_startCommandId : {}", m_startCommandId);
			throw new RuntimeException(new DScabiException("m_startCommandId is not set. m_startCommandId : " + m_startCommandId, "CRN.RUN.1"));			
			// Not used return;
		}
		if (m_endCommandId <= 0) {
			log.debug("run() m_endCommandId is not set. m_endCommandId : {}", m_endCommandId);
		...
		if (m_startCommandId > m_endCommandId) {
			log.debug("run() m_startCommandId is greater than m_endCommandId. m_startCommandId : {}, m_endCommandId : {}", m_startCommandId, m_endCommandId);
	...
	void get() {
		...
		if (m_futureHttpResponse != null) {
			...
			try {
				httpResponse = DataNoBlock.get(m_futureHttpResponse);
