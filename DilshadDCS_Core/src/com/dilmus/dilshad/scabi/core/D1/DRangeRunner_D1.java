 ...
 * All Rights Reserved.
 * Created 29-Feb-2016
 * File Name : DRangeRunner_D1.java
 */
 ...
 *
 */
public class DRangeRunner_D1 implements Runnable {

	private final Logger log = LoggerFactory.getLogger(DRangeRunner_D1.class);
	...
	public DRangeRunner_D1(DCompute compute, long startCRun, long endCRun) throws DScabiException {
	...
	public void run() {
		...
		LinkedList<DComputeAsyncRun_D1> listBlockedCRun = new LinkedList<DComputeAsyncRun_D1>();

		ListIterator<DComputeAsyncRun_D1> itr = null;
		try {
		...
		ListIterator<DComputeAsyncRun_D1> itr2 = null;
		try {
		...
		LinkedList<DComputeAsyncRun_D1> allowedCRunList = new LinkedList<DComputeAsyncRun_D1>();
		boolean check = true;
		while (check) {
			
			for (DComputeAsyncRun_D1 crun : listBlockedCRun) {
				DComputeNoBlock cnb = crun.getComputeNB();
				boolean proceed = false;
				synchronized(cnb) {
					if (cnb.isAllowed()) {
			...
			for (DComputeAsyncRun_D1 crun : allowedCRunList) {
