 ...
 * All Rights Reserved.
 * Created 24-Feb-2016
 * File Name : DComputeSyncRun_D1.java
 */
...
public class DComputeSyncRun_D1 implements Runnable {

	private final Logger log = LoggerFactory.getLogger(DComputeSyncRun_D1.class);
	private DComputeSyncConfig_D1 m_config = null;
	private DComputeBlock_D1 m_computeBlock = null;
	...
	public int setComputeBlock(DComputeBlock_D1 computeBlock) {
	...
	public void doRun() throws IOException, DScabiException {
		...
		*/
		...
		try {
			...
			if (DComputeSyncConfig_D1.OBJECT == m_config.getConfigType()) {
			...
			} else if (DComputeSyncConfig_D1.CLASS == m_config.getConfigType()){
				log.debug("doRun() Executing for Class");
				result = m_computeBlock.executeClass(m_config.getComputeClass());
				log.debug("doRun() result is : {}", result);
				
			} else if (DComputeSyncConfig_D1.CODE == m_config.getConfigType()){
				log.debug("doRun() Executing for Code");
				result = m_computeBlock.executeCode(m_config.getComputeCode());
				log.debug("doRun() result is : {}", result);
			} else if (DComputeSyncConfig_D1.CLASSNAMEINJAR == m_config.getConfigType()){
				log.debug("doRun() Executing for Class Name In Jar");
				result = m_computeBlock.executeClassNameInJar(m_config.getClassNameInJarFilePath(), m_config.getClassNameInJar());
			} else {
	...
	public void run() {
		...
		synchronized (m_computeBlock) {
			try {
				...
				m_computeBlock.setConfig(m_config);
		        doRun();
		    } catch (Throwable e) {
