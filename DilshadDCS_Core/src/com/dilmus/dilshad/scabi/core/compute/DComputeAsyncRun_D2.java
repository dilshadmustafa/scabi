 ...
 * All Rights Reserved.
 * Created 29-Feb-2016
 * File Name : DComputeAsyncRun_D2.java
 */
...
public class DComputeAsyncRun_D2 {

	private final Logger log = LoggerFactory.getLogger(DComputeAsyncRun_D2.class);
			...
			try {
				...
				*/
				...
				if (DComputeAsyncConfig.OBJECT == m_config.getConfigType()) {
					log.debug("submitTask() Executing for Object");
					futureHttpResponse = m_computeNB.executeObject(m_config.getComputeUnit());
					
				} else if (DComputeAsyncConfig.CLASS == m_config.getConfigType()){
					log.debug("submitTask() Executing for Class");
					futureHttpResponse = m_computeNB.executeClass(m_config.getComputeClass());
						
				} else if (DComputeAsyncConfig.CODE == m_config.getConfigType()){
					log.debug("submitTask() Executing for Code");
					futureHttpResponse = m_computeNB.executeCode(m_config.getComputeCode());
					
				} else if (DComputeAsyncConfig.CLASSNAMEINJAR == m_config.getConfigType()){
					log.debug("submitTask() Executing for Class Name In Jar");
					futureHttpResponse = m_computeNB.executeClassNameInJar(m_config.getClassNameInJarFilePath(), m_config.getClassNameInJar());
