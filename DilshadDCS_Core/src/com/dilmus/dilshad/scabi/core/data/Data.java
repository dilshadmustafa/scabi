...
public class Data implements Runnable {
	...
	// Not used private boolean m_isSplitSet = false;
	// Not used private long m_startSplit = -1;
	// Not used private long m_endSplit = -1;
	...
	*/
	...
	public Data(DMeta meta, String dataId, DataUnit dataUnitObj) throws DScabiException {
	...
	public Data(DMeta meta, String dataId, DPartitioner partitionerObj) throws DScabiException {
	...
	public Data(DMeta meta, String dataId) {
	...
	public int initialize() throws InterruptedException {
		...
		// Don't reset m_threadPool.shutdownNow();
		// Don't reset m_threadPool.awaitTermination(10, TimeUnit.MINUTES);
		// Don't reset m_threadPool = null;

		// Don't reset m_commandID = 2;
		// Don't reset m_commandMap.clear();
		// Don't reset m_jsonStrInput = m_emptyJsonStr;
		// Don't reset m_outputMap = null;
		
		// Don't reset m_maxSplit = 1;
		// Don't reset m_maxRetry = 3; //0;
		// Don't reset m_maxThreads = 0; //1;
		// Don't reset m_splitTotal = 0;
		
		m_configNode = null;
		
		// Not used m_isSplitSet = false;
		// Not used m_startSplit = -1;
		// Not used m_endSplit = -1;

		m_isPerformInProgress = false;
		// Don't reset m_crunListReady = false;
		m_futureListReady = false;

		synchronized (this) {
			
			// Don't reset m_cconfigList.clear();
			// m_crunList is not thread safe and RetryMonitor is also using it
			// So it is cleared after thread pool shutdown
			// Don't reset m_crunList.clear();
			// Don't reset m_cnbList.clear();
					
			m_futureList.clear();
			m_futureRRunMap.clear();
			
			// Don't reset m_isJarFilePathListSet = false;
			// Don't reset m_jarFilePathList.clear();
			
			// Don't reset m_isComputeUnitJarsSet = false;
			// Don't reset m_dcl = null;
			
			// Don't reset m_crunListSize = 0;
			// Don't reset m_cnbListSize = 0;
			m_futureListSize = 0;
		}
		
		// Don't reset m_dataUnitType = 0;
		m_configNodeType = 0;
		// Don't reset m_dataId = null;
		// Don't reset m_dataUnitClass = null;
		// Don't reset m_dataUnit = null;
	...
	private Data setDataUnitClass(Class<? extends DataUnit> cls) throws DScabiException, IOException {
		...
		if (m_isJarFilePathListSet) {
			config.setJarFilePathFromList(m_jarFilePathList);
			// Don't reset m_isJarFilePathListSet = false;
			// Don't reset m_jarFilePathList.clear();
		}

		if (m_isComputeUnitJarsSet) {
			config.setComputeUnitJars(m_dcl);
			// Don't reset m_isComputeUnitJarsSet = false;
			// Don't reset m_dcl = null;
		}
		*/
	...
	private Data setPartitionerClass(Class<? extends DPartitioner> cls) throws DScabiException, IOException {
		...
		DPartitionerConfig config = new DPartitionerConfig(cls);
		m_configNode = new DataAsyncConfigNode(config);
	...
	private Data setPartitionerObject(DPartitioner unit) throws DScabiException, IOException {
		...
		m_configNodeType = DataAsyncConfigNode.CNT_PARTITIONER_CONFIG;
		DPartitionerConfig config = new DPartitionerConfig(unit);
		m_configNode = new DataAsyncConfigNode(config);
	...
	private int setForOperatorConfig_1_1(DOperatorConfig_1_1 config) {
	...
	private int setForOperatorConfig_1_2(DOperatorConfig_1_2 config) {
	...
	private int setForOperatorConfig_2_1(DOperatorConfig_2_1 config) {
	...
	private int setForOperatorConfig_2_2(DOperatorConfig_2_2 config) {
	...
	public Data repartition(String newDataId, DPartitioner partitioner) throws DScabiException {
	...
	private Data operate(String dataId1, String dataId2, IOperator unit) throws DScabiException, IOException {
		...
		if (m_configNode != null) {
			...
			} else if (DataAsyncConfigNode.CNT_OPERATOR_CONFIG_2_1 == m_configNode.getConfigNodeType()) {
				setForOperatorConfig_2_1(m_configNode.getOperatorConfig_2_1());
			} else if (DataAsyncConfigNode.CNT_OPERATOR_CONFIG_2_2 == m_configNode.getConfigNodeType()) {
				setForOperatorConfig_2_2(m_configNode.getOperatorConfig_2_2());
			}
		...
		DOperatorConfig_1_1 config = new DOperatorConfig_1_1(unit);
		m_configNode = new DataAsyncConfigNode(config);
		m_dataId1 = dataId1;
		m_dataId2 = dataId2;
	...
	private Data operate(String dataId1, String dataId2, IOperator2 unit) throws DScabiException, IOException {
		...
		DOperatorConfig_1_2 config = new DOperatorConfig_1_2(unit);
		m_configNode = new DataAsyncConfigNode(config);
		m_dataId1 = dataId1;
		m_dataId2 = dataId2;
	...
	private Data operate(String dataId1, IOperator unit) throws DScabiException, IOException {
		// Source and Target Data Id will be same
		if (m_isPerformInProgress) {
			throw new DScabiException("Perform already in progress", "DAA.OPE.1");
		}
		...
		m_configNodeType = DataAsyncConfigNode.CNT_OPERATOR_CONFIG_2_1;
		DOperatorConfig_2_1 config = new DOperatorConfig_2_1(unit);
		m_configNode = new DataAsyncConfigNode(config);
		m_dataId1 = dataId1;
		m_dataId2 = dataId1;
	...
	private Data operate(String dataId1, IOperator2 unit) throws DScabiException, IOException {
		// Source and Target Data Id will be same
		if (m_isPerformInProgress) {
			throw new DScabiException("Perform already in progress", "DAA.OPE.1");
		}
		...
		m_configNodeType = DataAsyncConfigNode.CNT_OPERATOR_CONFIG_2_2;
		DOperatorConfig_2_2 config = new DOperatorConfig_2_2(unit);
		m_configNode = new DataAsyncConfigNode(config);
		m_dataId1 = dataId1;
		m_dataId2 = dataId1;
	...
	private Data shuffle(String dataId1, String dataId2, IShuffle unit) throws DScabiException, IOException {
		...
		DShuffleConfig_1_1 config = new DShuffleConfig_1_1(unit);
		m_configNode = new DataAsyncConfigNode(config);
	...
	private Data shuffle(String dataId1, String dataId2, IShuffle2 unit) throws DScabiException, IOException {
		...
		DShuffleConfig_1_2 config = new DShuffleConfig_1_2(unit);
		m_configNode = new DataAsyncConfigNode(config);
		config.setSourceDataId(dataId1);
		config.setTargetDataId(dataId2);
	...
	private Data sort(String dataId1, String dataId2, IComparator<?> unit) throws DScabiException, IOException {
		...
		DComparatorConfig_1_1 config = new DComparatorConfig_1_1(unit);
		m_configNode = new DataAsyncConfigNode(config);
		config.setSourceDataId(dataId1);
		config.setTargetDataId(dataId2);
	...
	public Data perform() throws DScabiException, IOException {
		...
		*/
		...
		if (DataAsyncConfigNode.CNT_DATAUNIT_CONFIG == m_commandMap.get("1").getConfigNodeType()) {
			...
			m_commandMap.get("1").getDataUnitConfig().setJobId(jobId);
			...
			m_commandMap.get("1").getPartitionerConfig().setJobId(jobId);
